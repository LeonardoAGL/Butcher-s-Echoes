...
if (global.player_state == "normal") {
    ...
    // Direccion teclado
	hsp = (input_right() - input_left()) * spd;
	vsp = (input_down() - input_up()) * spd;
    ...
    if (hsp != 0 || vsp != 0) {  //Caminando
		...
		if (step_timer >= step_interval) {
			play_sfx("snd_walk"); // Cridem el so des de la funció central
			step_timer = 0;
		}
		...
		switch (last_direction) {
            ...
            case "up": sprite_index = spr_player_walk_up; break;
        }
    } else {  //Quieto
		step_timer = 0;
...
// === Sprite idle si no està en estat normal ===
if (global.player_state != "normal") {
    switch (last_direction) {
        case "right": sprite_index = spr_player_idle_right; break;
        case "left":  sprite_index = spr_player_idle_left;  break;
